# This script open files best_kernels_csv_per_feature.csv where are the best params for each kernel for all features
# It gets unique params from that file for kernels rbf, poly and sigmoid and then get the values for each params for all features
...
for kernel in kernel_list:
    ...
    for file in listdir(path):
        dir = path + "/" + file
        if file.startswith("grid") and file.endswith(".csv"):
    ...
    details.to_csv(path + '/best_' + kernel + '.csv', header=True, index = False, mode='w')
