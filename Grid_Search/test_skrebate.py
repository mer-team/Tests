...
clf = GridSearchCV(
        ...
        cv=cv, scoring='f1_macro', n_jobs=46)
...
scoresdf.to_csv('./scoresdf_turf.csv', index = False, columns=columns_to_write, mode='w')
...
while counter <= len(headers):
    ...
    scoresRow.to_csv('./scoresdf_turf.csv', header=False, index = False, columns=columns_to_write, mode='a')
